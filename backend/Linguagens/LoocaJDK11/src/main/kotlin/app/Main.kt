...
open class Main {
    companion object{
        @JvmStatic fun main(args: Array<String>){
            ...
            JOptionPane.showMessageDialog(null, "Bem-vindo ao console Greeneye!")
            JOptionPane.showMessageDialog(null, "O sistema da sua maquina é ${sistema.sistemaOperacional}")
            ...
            while (true){
                ...
                when(escolha){
                    "1" -> {
                        JOptionPane.showMessageDialog(null, "Uso do processador: ${"%.0f".format(usoProcessador)}%")
                        leituraRepositorio.inserirLeitura(novaLeitura)}
                    "2" -> {
                        JOptionPane.showMessageDialog(null, "Memória RAM total (GB): ${"%.2f".format(ramTotal)}")
                        leituraRepositorio.inserirLeitura(novaLeitura)}
                    "3" -> {
                        JOptionPane.showMessageDialog(null, "Memória RAM utilizada (GB): ${"%.2f".format(ramUso)}")
                        leituraRepositorio.inserirLeitura(novaLeitura)}
                    "4" -> {
                        JOptionPane.showMessageDialog(null, "Memória dispónível (GB): ${"%.2f".format(ramDisponivel)}")
                        leituraRepositorio.inserirLeitura(novaLeitura)}
                    "5" -> {
                        JOptionPane.showMessageDialog(
                            null,
                            "Tamanho do(s) disco(s) (GB):Disco 1 - ${"%.2f".format(tamanhoDiscos)} / Disco 2 - ${
                                "%.2f".format(tamanhoDiscos1)
                            }"
                        )
                        leituraRepositorio.inserirLeitura(novaLeitura)}
                    "6" -> {
                        JOptionPane.showMessageDialog(
                            null, "Todos:\r\nUso do Processador:${"%.0f".format(usoProcessador)}% \r\n" +
                                    ...
                                    "\r\nTamanho disco(s):Disco 1 - ${"%.2f".format(tamanhoDiscos)} / Disco 2 - ${
                                        "%.2f".format(
                                            tamanhoDiscos1
                                        )
                                    }"
